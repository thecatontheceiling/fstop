...
CPhysicsCloneArea *CPhysicsCloneArea::CreatePhysicsCloneArea( CProp_Portal *pFollowPortal )
{
	...
	pCloneArea->vLocalMins = Vector(3.0f,
							   -pFollowPortal->GetHalfWidth() * PHYSICSCLONEAREASCALE,
							   -pFollowPortal->GetHalfHeight() * PHYSICSCLONEAREASCALE );

	pCloneArea->vLocalMaxs = Vector( pFollowPortal->GetHalfHeight() * PHYSICSCLONEAREASCALE,  //x is the forward which is fairly thin for portals, replacing with halfheight
									 pFollowPortal->GetHalfWidth() * PHYSICSCLONEAREASCALE,
									 pFollowPortal->GetHalfHeight() * PHYSICSCLONEAREASCALE );
