...
BEGIN_DATADESC( CDynamicProp )
	...
	DEFINE_KEYFIELD( m_nGlowRenderMode, FIELD_COLOR32, "glowrendermode" ),
	...
	DEFINE_INPUTFUNC( FIELD_FLOAT, "SetGlowRenderMode", InputSetGlowRenderMode ),
...
IMPLEMENT_SERVERCLASS_ST(CDynamicProp, DT_DynamicProp)
	...
	SendPropInt( SENDINFO( m_nGlowRenderMode ) ),
END_SEND_TABLE()
...
CDynamicProp::CDynamicProp()
{
	...
	m_nGlowRenderMode = 3; // GLOW_RENDER_MODE_ALWAYS
}
...
void CDynamicProp::InputSetGlowRenderMode( inputdata_t &inputdata )
{
	m_nGlowRenderMode.Set( (int)inputdata.value.Float() );
}
