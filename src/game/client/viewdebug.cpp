...
static void OverlayShowTexture( const char* textureName, float scale )
{
	...
	// if this material doesn't exist this will work anyway using ___error
	// ___error is created in code in CMaterialSystem::CreateDebugMaterials()
	pMaterial = materials->FindMaterial( "debug/debugdrawtexture", TEXTURE_GROUP_OTHER, true );
	...
	x = 0.0f;// (nViewportWidth - w) * 0.5f;
	y = 0.0f;// (nViewportHeight - h) * 0.5f;
