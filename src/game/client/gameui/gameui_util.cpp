...
CON_COMMAND(cl_eval_expr, "Evaluate an expression with CExpressionEvaluator.")
{
	if ( args.ArgC() < 1 )
	{
		Msg( "Error: no expression provided.\n" );
		return;
	}
	CExpressionEvaluator *exprs = new CExpressionEvaluator();

	bool result = false;

	if ( exprs->Evaluate( result, args.ArgS() ) )
		Msg( "True\n" );
	else
		Msg( "False\n" );
