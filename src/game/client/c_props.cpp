...
BEGIN_NETWORK_TABLE( CDynamicProp, DT_DynamicProp )
	...
	RecvPropInt( RECVINFO( m_nGlowRenderMode ) ),
END_NETWORK_TABLE()
...
: m_GlowObject( this, Vector( 1.0f, 1.0f, 1.0f ), 0.0f, false, false )
{
	...
	m_nGlowRenderMode = GLOW_RENDER_MODE_ALWAYS;
}
...
void C_DynamicProp::UpdateGlow( void )
{
	...
	// Start glowing
	//m_GlowObject.SetRenderFlags( false, m_bShouldGlow );
	m_GlowObject.SetRenderMode( (GlowRenderMode_t)m_nGlowRenderMode );
...
void C_DynamicProp::DisableGlow( void )
{
	m_bShouldGlow = false;
