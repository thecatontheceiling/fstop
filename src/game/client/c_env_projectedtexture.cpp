...
IMPLEMENT_CLIENTCLASS_DT( C_EnvProjectedTexture, DT_EnvProjectedTexture, CEnvProjectedTexture )
	...
	RecvPropFloat(	 RECVINFO( m_flShadowSoftness ) ),
	RecvPropBool(	 RECVINFO( m_bUberlight ) ),
	RecvPropArray3( RECVINFO_ARRAY( m_UberlightParams ), RecvPropFloat( RECVINFO( m_UberlightParams[0] ) ) ),
	//RecvPropArray( RecvPropFloat( RECVINFO_ARRAY( m_UberlightParams ) ), m_UberlightParams ),
...
//	strcpy( pEnt->m_SpotlightTextureName, "particle/rj" );
	...
	pEnt->m_flShadowSoftness = 0.2f;
	pEnt->m_bUberlight = false;
	//float defaults[11] = {2.0f, 100.0f, 10.0f, 650.0f, 0.0f, 0.0f, 0.3f, 0.05f, 0.3f, 0.05f, 0.8f};
...
{
	//ConMsg( "m_UberlightParams[10]: %f\n", m_UberlightParams[10] );
...
#pragma message("OPTIMIZATION: this should be made SIMD")
		...
		state.m_flShadowFilterSize = m_flShadowSoftness;

		// TODO: volumetrics maybe?
		//state.m_bVolumetric = true;

		state.m_bUberlight = m_bUberlight;
		
		state.m_uberlightState = UberlightState_t();
		state.m_uberlightState.m_fNearEdge = m_UberlightParams[0];
		state.m_uberlightState.m_fFarEdge = m_UberlightParams[1];
		state.m_uberlightState.m_fCutOn = m_UberlightParams[2];
		state.m_uberlightState.m_fCutOff = m_UberlightParams[3];
		state.m_uberlightState.m_fShearx = m_UberlightParams[4];
		state.m_uberlightState.m_fSheary = m_UberlightParams[5];
		state.m_uberlightState.m_fWidth = m_UberlightParams[6];
		state.m_uberlightState.m_fWedge = m_UberlightParams[7];
		state.m_uberlightState.m_fHeight = m_UberlightParams[8];
		state.m_uberlightState.m_fHedge = m_UberlightParams[9];
		state.m_uberlightState.m_fRoundness = m_UberlightParams[10];
