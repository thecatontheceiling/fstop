...
extern CPortalRender& GetPortalRender();


inline CPortalRenderable *CPortalRenderable::FindRecordedPortal( int nPortalId )
{ 
	return GetPortalRender().FindRecordedPortal( nPortalId );
}
...
public:
	...
	{
		GetPortalRender().AddPortalCreationFunc( szPortalType, creationFunction );
	}
};
...
{
	GetPortalRender().m_iViewRecursionLevel = iViewRecursionLevel;
}

inline void CPortalRenderable::SetRemainingViewDepth( int iRemainingViewDepth )
{
	GetPortalRender().m_iRemainingPortalViewDepth = iRemainingViewDepth;
}

inline void CPortalRenderable::SetViewEntranceAndExitPortals( CPortalRenderable *pEntryPortal, CPortalRenderable *pExitPortal )
{
	GetPortalRender().m_pRenderingViewForPortal = pEntryPortal;
	GetPortalRender().m_pRenderingViewExitPortal = pExitPortal;
}
