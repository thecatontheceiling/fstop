...
CPortalRender& GetPortalRender()
{
	return s_PortalRender;
}
...
void CPortalRenderable::BeginPortalPixelVisibilityQuery(void)
{
	...
	if (GetPortalRender().ShouldUseStencilsToRenderPortals()) //this function exists because we require help in texture mode, we need no assistance in stencil mode. Moreover, doing the query twice will probably fubar the results
		return;

	PortalViewIDNode_t *pCurrentPortalViewNode = GetPortalRender().m_PortalViewIDNodeChain[GetPortalRender().m_iViewRecursionLevel]->ChildNodes[m_iPortalViewIDNodeIndex];
...
void CPortalRender::DrawPortalsToTextures(CViewRender *pViewRender, const CViewSetup &cameraView)
{
	...
	int iNumRenderablePortals = GetPortalRender().m_ActivePortals.Count();
	...
	for (int i = 0; i != iNumRenderablePortals; ++i)
	{
		CPortalRenderable *pCurrentPortal = GetPortalRender().m_ActivePortals[i];
