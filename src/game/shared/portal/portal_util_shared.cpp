...
void UTIL_Portal_Triangles( const Vector &ptPortalCenter, const QAngle &qPortalAngles, const unsigned short iHalfWidth, const unsigned short iHalfHeight, Vector pvTri1[ 3 ], Vector pvTri2[ 3 ] )
{
	// Get points to make triangles
	Vector vRight, vUp;
	AngleVectors( qPortalAngles, NULL, &vRight, &vUp );

	Vector vTopEdge = vUp * iHalfHeight;
	Vector vBottomEdge = -vTopEdge;
	Vector vRightEdge = vRight * iHalfWidth;
...
{
	UTIL_Portal_Triangles( pPortal->GetAbsOrigin(), pPortal->GetAbsAngles(), pPortal->GetHalfWidth(), pPortal->GetHalfHeight(), pvTri1, pvTri2 );
}
			...
			if( pLinkedPortal != NULL )
			{
				...
				if( fDirectDist < fMinDist )
				{
					...
					if( bStraightLine ) //if we're not already doing some crazy wrapping, find an intersection point
					{
						...
						if( (fAbsRight > pLinkedPortal->GetHalfWidth() ) ||
							(fAbsUp > pLinkedPortal->GetHalfWidth() ) )
							bStraightLine = false;
						...
						if( bStraightLine == false )
						{
							...
							if ( fAbsRight > pLinkedPortal->GetHalfWidth() )
							{
								fNormalizer = fAbsRight/pLinkedPortal->GetHalfWidth();

								if( fAbsUp > pLinkedPortal->GetHalfHeight() )
								{
									float fUpNormalizer = fAbsUp/pLinkedPortal->GetHalfHeight();
							...
							{
								fNormalizer = fAbsUp/pLinkedPortal->GetHalfHeight();
							}
...
{
	return IntersectRayWithOBB( ray, pPortal->GetAbsOrigin(), pPortal->GetAbsAngles(), pPortal->GetLocalMins(),  pPortal->GetLocalMaxs(), 0.0f, pTrace );
}
#endif
...
bool UTIL_IsBoxIntersectingPortal( const Vector &vecBoxCenter, const Vector &vecBoxExtents, const Vector &ptPortalCenter, const QAngle &qPortalAngles,
								   const unsigned short iHalfWidth, const unsigned short iHalfHeight, float flTolerance )
{
	Vector pvTri1[ 3 ], pvTri2[ 3 ];

	UTIL_Portal_Triangles( ptPortalCenter, qPortalAngles, iHalfWidth, iHalfHeight, pvTri1, pvTri2 );
...
bool UTIL_IsBoxIntersectingPortal( const Vector &vecBoxCenter, const Vector &vecBoxExtents, const CProp_Portal *pPortal, float flTolerance )
{
	...
	return UTIL_IsBoxIntersectingPortal( vecBoxCenter, vecBoxExtents, pPortal->GetAbsOrigin(), pPortal->GetAbsAngles(), pPortal->GetHalfWidth(), pPortal->GetHalfHeight(), flTolerance );
}
...
void UTIL_Portal_NDebugOverlay( const Vector &ptPortalCenter, const QAngle &qPortalAngles, int r, int g, int b, int a, bool noDepthTest, float duration, unsigned short iHalfWidth, unsigned short iHalfHeight )
{
#ifndef CLIENT_DLL
	Vector pvTri1[ 3 ], pvTri2[ 3 ];
...
{
#ifndef CLIENT_DLL
	UTIL_Portal_NDebugOverlay( pPortal->GetAbsOrigin(), pPortal->GetAbsAngles(), r, g, b, a, noDepthTest, duration, pPortal->GetHalfWidth(), pPortal->GetHalfHeight() );
#endif //#ifndef CLIENT_DLL
}
...
bool UTIL_Portal_EntityIsInPortalHole( const CProp_Portal *pPortal, CBaseEntity *pEntity )
{
	...
	return OBBHasFullyContainedIntersectionWithQuad( vForward, vRight, vUp, ptOBBCenter, 
		...
		vPortalRight, pPortal->GetHalfWidth() + 1.0f, vPortalUp, pPortal->GetHalfHeight() + 1.0f );
}
