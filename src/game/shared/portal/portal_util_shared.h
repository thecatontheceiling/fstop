...
bool UTIL_IsBoxIntersectingPortal( const Vector &vecBoxCenter, const Vector &vecBoxExtents, const Vector &ptPortalCenter, const QAngle &qPortalAngles, const unsigned short iHalfWidth, const unsigned short iHalfHeight, float flTolerance = 0.0f );
...
void UTIL_Portal_NDebugOverlay( const Vector &ptPortalCenter, const QAngle &qPortalAngles, int r, int g, int b, int a, bool noDepthTest, float duration, unsigned short iHalfWidth = PORTAL_DEFAULT_HALF_WIDTH, unsigned short iHalfHeight = PORTAL_DEFAULT_HALF_HEIGHT );
